...
A Listagem~\ref{lst:syntactic_errors} apresenta alguns exemplos de erros sintáticos.
...
símbolos, a verificação de tipos e a consistência ação e domínio, que serão descritas a seguir. 
...
% O significado de uma ação
% depende do domínio do alvo, isto é, o tipo do dispositivo indicado
% pelo primeiro parâmetro. A análise procede em etapas: (i) determina o
% domínio do alvo, exigindo que, num alvo em lista, todas as
% entidades compartilhem o mesmo domínio; (ii) verifica que a ação
% é definida para aquele domínio; (iii) valida os argumentos,
% cada argumento precisa ser aceito por aquela ação naquele
...
O \texttt{jass} cumpre os objetivos propostos, oferecendo uma sintaxe
...
% Como , destacam-se a ampliação do registro de ações e domínios, o
% suporte a novas formas de gatilho e condição, a integração com a
% validação nativa do Home Assistant

% todas
% as automações de exemplo são transpiladas para YAML válido e carregável,
% validando o pipeline de ponta a ponta. Como limitações conhecidas, o
