"""Runtime value types carried by the AST.

These wrap the literal forms that need structure beyond a plain Python scalar —
durations, time-of-day, colors and entity references — with parsers from their
source text and helpers for lowering to Home Assistant YAML.
"""
...
class Duration:
    """A span of time, e.g. ``1h30m`` or ``45s``."""
    ...
    def as_hms(self) -> str:
        """Render as ``HH:MM:SS`` (Home Assistant's duration string form)."""
...
@dataclass(frozen=True)
class TimeOfDay:
    """A wall-clock time, written ``@HH:MM:SS``."""
...
@dataclass(frozen=True)
class Color:
    """An RGB color, written ``rgb(r, g, b)``."""
...
@dataclass(frozen=True)
class EntityRef:
    """A reference to a Home Assistant entity, ``domain.object_id``."""
